...
export class PropertyListingService {
  ...
  async transformAndSaveProperty(data: {
    ...
    try {
      ...
      const propertyData: Partial<PropertyListing> = {
        ...
        details: {
          capacity: {
            ...
            bathrooms: 
              accommodation.Features.Distribution.BathroomWithBathtub || '0' +
              accommodation.Features.Distribution.BathroomWithShower || '0',
          ...
          amenities: {
            ...
            hasWashingMachine: accommodation.Features.HouseCharacteristics.Kitchen.WashingMachine === 'true',
      ...
      const existingListing = await this.databaseService.findByPropertyId({ propertyId: propertyData.propertyId as any });
      if (existingListing) {
  ...
  private getDescriptionInEnglish(descriptionData: any): string {
    const englishDescription = descriptionData?.InternationalizedItem?.find(
  ...
  private transformImages(picturesData: any): Array<any> {
    ...
    const pictures = Array.isArray(picturesData.Picture) 
